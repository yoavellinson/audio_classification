...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = le.transform([os.path.split(wav_path)[0].split('/')[-1]])[0]
        # generate a batch of time data
        X = np.empty((1, int(self.sr*self.dt), 1), dtype=np.float32)
        Y = np.empty((1, self.n_classes), dtype=np.float32)
        wav,rate = torchaudio.load(wav_path)
        X[0,] = wav.reshape(-1, 1)
        Y[0,] = to_categorical(label, num_classes=self.n_classes)
